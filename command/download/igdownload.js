...
module.exports = {
	...
	/*	if (command == "ig" || command == "instagram")
		...
		delete ig.medias;
		//var parse = 'L Ξ X A', //await rzky.tools.parseResult(ig.user, { title: "Instagram Download" });
		//await conn.sendFile(msg.from, img, "ig.jpg", parse, msg);
		for (let i of result) {
			await conn.sendFile(msg.from, i.url, ig.user.username + i.fileType, `*L Ξ X A*\n*${ig.user.username}*`, msg);
		}
			} catch (err) {
			console.log(err);
