...
/*
		...
		conn.sendMessage = async (jid, content, options = { isTranslate: false }) => {
...
conn.sendMessage(msg.from, templateMessage)
    ...
    if (msg.quoted && msg.quoted.key.fromMe ) {
	  //if (!msg.quoted.key.fromMe) return 0
	  
	  if (msg.quoted.key.fromMe && /Happy\sAnswering/i.test(msg.quoted.text)) return 0;
    if (type === 'stickerMessage') return 
    let fetch = require('node-fetch')
    balasan  = await fetch(`https://api.lolhuman.xyz/api/simi?apikey=Sad-Bot&text=${encodeURIComponent(msg.body)}`, {method: 'get'})
    let pesanB = await balasan.json()
    //if (json.success) m.reply(json.success)
    msg.reply(pesanB.result.replace('simsimi', 'Lexa').replace('Simsimi', 'lexa').replace('simi', 'Lexa').replace('Simi', 'Lexa').replace('sim', 'Lexa'))
    }
   } catch (e) {
     console.log('Error' + e)
   }
  ...
  return list[Math.floor(Math.random() * list.length)];
   ...
   console.log('Error' + e)
    ...
    /*
     const templateMessage = {
       text: "",
       footer: `${pesanB.success.replace('simsimi', 'Lexa').replace('Simsimi', 'lexa').replace('simi', 'Lexa').replace('Simi', 'Lexa').replace('sim', 'Lexa')}\n`,
       templateButtons: []
     }
		...
		if (
			...
			try {
				...
				if (rz.url) {
					...
					await conn.sendFile(msg.from, res.url, Date.now() + "media.mp4", '', msg);
					...
					await conn.sendMessage(
						msg.from,
						{
							...
							caption: '*L Ξ X A*\n*' + rz.author + '* ' + rz.desc, //await rzky.tools.parseResult(rz, { title: "Auto Download" }),
							templateButtons: [
